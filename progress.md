...
## 2026-09-19 - Task: 网页版补齐激光模式与皮肤商店

### What was done

- 网页增强版新增第 6 个模式「激光」：上下移动的激光门，周期性闪灭（约 2 秒亮、0.5 秒灭），灭时可穿过，得分 ×2；参数由 Qt 版按毫秒换算为本文件的帧单位。
- 金币改为自带坐标的自由实体，不再挂在管道上，激光模式因此也能掉落金币；风暴模式金币不再随管道摆动，与 Qt 版行为一致。
- 新增皮肤商店：8 款皮肤（含霓虹发光与彩虹变色），用累计金币购买并装备，`fb_skins_owned` 与 `fb_skin` 持久化，补上与加钱对称的扣款路径。
- 菜单重排以容纳 6 张卡片：卡片改为 56 高、间距 63，金币总数移至顶部，底部新增商店入口按钮。
- 小鸟上色由模式色改为皮肤色；彩虹皮肤按时间生成十六进制颜色（现有颜色辅助函数只接受 hex），发光皮肤复用夜航的外发光逻辑。

### Testing

- 浏览器驱动测试 21 项全部通过。
- 激光机制逐项验证：生成的门带 176 间隙与非零摆幅、从右侧屏外入场；闪烁采样 20 亮 5 灭；穿过间隙安全、碰到亮光致死、光灭时同一位置可安全穿过。
- 皮肤商店验证：购买扣款 250→230、`fb_skins_owned` 与 `fb_skin` 正确落盘、刷新后仍装备、金币不足时余额与装备均不变。
- 彩虹皮肤颜色随时间循环（#ff5252 → #55ff52 → #5259ff）且均为合法十六进制；固定皮肤颜色不变。
- iPhone 13 模拟下帧间隔中位数与 p95 均为 16.70ms（60fps 无掉帧），与 Qt WebAssembly 版在手机上的表现形成对比。
- 菜单、商店、激光模式游戏中截图逐张检查；修正了商店提示文案与底排格子重叠的问题。

### Notes

- `flappy-bird-enhanced.html`：新增激光模式、皮肤商店、自由金币与皮肤上色；菜单重排。
- `README.md`：网页版改为 6 模式并补充皮肤商店与 `B` 键；在线游玩章节说明手机自动分流。
- `progress.md`：追加本轮记录。
